...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(CONTROL_SYSTEM_V3_M) == (NULL)) &&
    !rtmGetStopRequested(CONTROL_SYSTEM_V3_M);
  while (runModel) {
    ...
    stopRequested = !((rtmGetErrorStatus(CONTROL_SYSTEM_V3_M) == (NULL)) &&
                      !rtmGetStopRequested(CONTROL_SYSTEM_V3_M));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(CONTROL_SYSTEM_V3_M, "stopping the model");
}
...
int app_main(int argc, char **argv)
{
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(CONTROL_SYSTEM_V3_M, 0);
  ...
  /* Initialize model */
  CONTROL_SYSTEM_V3_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(CONTROL_SYSTEM_V3_M->extModeInfo, &rtmGetTFinal
    (CONTROL_SYSTEM_V3_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(CONTROL_SYSTEM_V3_M, true);
    }
  }
