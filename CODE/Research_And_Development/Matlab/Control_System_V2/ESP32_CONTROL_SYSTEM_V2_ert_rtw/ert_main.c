...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(ESP32_CONTROL_SYSTEM_V2_M) == (NULL));
  while (runModel) {
    ...
    ESP32_CONTROL_SYSTEM_V2_step();

    /* Get model outputs here */

    /* Trigger External Mode event */
    extmodeEvent(0, currentTime);
    stopRequested = !((rtmGetErrorStatus(ESP32_CONTROL_SYSTEM_V2_M) == (NULL)));
...
{
  UNUSED(sig);
  rtmSetErrorStatus(ESP32_CONTROL_SYSTEM_V2_M, "stopping the model");
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  ESP32_CONTROL_SYSTEM_V2_terminate();
  extmodeReset();
...
int app_main(int argc, char **argv)
{
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ESP32_CONTROL_SYSTEM_V2_M, 0);
  ...
  /* Initialize model */
  ESP32_CONTROL_SYSTEM_V2_initialize();

  /* External Mode initialization */
  errorCode = extmodeInit(ESP32_CONTROL_SYSTEM_V2_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ESP32_CONTROL_SYSTEM_V2_M, true);
    }
  }
