 ...
 *  -------------------------------------------------------------------------
 *
 * Created: Mon Jul 29 19:47:56 2024
 */

#define S_FUNCTION_LEVEL               2
#define S_FUNCTION_NAME                sfAFMotor
  ...
  if (ssGetSimMode(S) != SS_SIMMODE_SIZES_CALL_ONLY) {
    ...
    if (inUnitIdReg != INVALID_UNIT_ID) {
      ...
      ssSetLocalErrorStatus(S,
                            "Invalid Unit provided for input port DIR of S-Function sfAFMotor");
      return;
    }
    ...
    if (inUnitIdReg != INVALID_UNIT_ID) {
      ...
      ssSetLocalErrorStatus(S,
                            "Invalid Unit provided for input port PWM of S-Function sfAFMotor");
      return;
    }
...
{
  const uint8_T *DIR = (uint8_T *) ssGetInputPortRealSignal(S, 0);
  const uint8_T *PWM = (uint8_T *) ssGetInputPortRealSignal(S, 1);
  sfAFMotor_Outputs_wrapper(DIR, PWM);
}
