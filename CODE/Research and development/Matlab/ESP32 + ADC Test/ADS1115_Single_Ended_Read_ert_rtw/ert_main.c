...
int app_main(int argc, char **argv)
{
  ...
  errorCode = extmodeInit(ADS1115_Single_Ended_Read_M->extModeInfo,
    &rtmGetTFinal(ADS1115_Single_Ended_Read_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ADS1115_Single_Ended_Read_M, true);
    }
  }
